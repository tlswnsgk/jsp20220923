...
-- Employees 테이블의 모든 레코드(행)을 LastName, BitrhDate열 조회
...
select lastname, birthdate from Employees;
-- select lastname,birthdate from employees; 데이터 베이스 존재 안 함

-- 모든 컬럼 : *
SELECT * FROM Customers;
SELECT CustomerID,CustomerName,ContactName,City,PostalCode,Country FROM Customers;
